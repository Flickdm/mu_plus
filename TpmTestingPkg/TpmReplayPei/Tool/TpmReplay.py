...
def _decode_variable_driver_config(data):

    driver_config = {}

    with BytesIO(data) as fs:

        for Esl in EfiSignatureDatabase(filestream=fs).EslList:
            ...
            if Esl.signature_type == EfiSignatureDataFactory.EFI_CERT_SHA256_GUID:
                ...
                for sig_list in Esl.signature_data_list:
...
def _decode_variable_authority(data) -> list:
...
def _decode_value_dispatch(event_type, name, data) -> dict:
    ...
    func = {
        ...
        # f"dbx+{EV_EFI_VARIABLE_DRIVER_CONFIG}": _decode_variable_driver_config, TODO
